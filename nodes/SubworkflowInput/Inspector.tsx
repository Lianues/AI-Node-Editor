...
const SubworkflowInputInspector: React.FC<
...
> = ({ node, updateNodeData, executionDetails, customTools }) => { // Added customTools
  ...
  const [portName, setPortName] = useState(node.data?.portName || '输入1');
  ...
  useEffect(() => {
    setPortName(node.data?.portName || '输入1');
  ...
  const handleNameBlur = () => {
    ...
    } else if (updateNodeData && !trimmedName && (node.data?.portName || '输入1') ) {
      setPortName(node.data?.portName || '输入1'); // Revert to old or default if cleared
    } else if (updateNodeData && !trimmedName) { // Allow setting to empty if explicitly desired
  ...
  if (!nodeDefinition) {
    return <p className={`text-sm ${inspectorTheme.warningText}`}>无法加载子程序输入节点的检查器：节点定义未找到。</p>;
  }
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <div>
          <label className={labelClass} htmlFor={`sw-input-portName-${node.id}`}>端口名称</label>
          <input
            id={`sw-input-portName-${node.id}`}
            type="text"
            ...
            aria-label="Subworkflow Input Port Name"
          />
          <p className={`text-xs ${inspectorTheme.labelText} mt-1`}>此名称将作为子程序实例节点上的一个输入端口显示。</p>
        </div>
        <div>
          <label className={labelClass} htmlFor={`sw-input-portDataType-${node.id}`}>端口类型</label>
          <select
            id={`sw-input-portDataType-${node.id}`}
            className={inputBaseClass}
            value={portDataType}
            onChange={handleDataTypeChange}
            onMouseDown={stopPropagationMouseDown}
            aria-label="Subworkflow Input Port Data Type"
          >
        ...
        <div className="flex items-center">
          <input
            id={`sw-input-isPortRequired-${node.id}`}
            type="checkbox"
          ...
          />
          <label htmlFor={`sw-input-isPortRequired-${node.id}`} className={checkboxLabelClass}>
            是否必须 (端口将显示为菱形)
          </label>
        </div>
      </div>
...
export default SubworkflowInputInspector;
