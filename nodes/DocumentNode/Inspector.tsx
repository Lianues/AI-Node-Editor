...
const DocumentNodeInspector: React.FC<SpecificNodeInspectorProps & { updateNodeData?: (nodeId: string, data: Record<string, any>) => void }> = ({ node, updateNodeData, executionDetails, customTools }) => {
  ...
  if (!nodeDefinition) {
    return <p className={`text-sm ${inspectorTheme.warningText}`}>无法加载全局变量节点的检查器：节点定义未找到。</p>;
  }
...
export default DocumentNodeInspector;